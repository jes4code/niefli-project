...
let cargarDatosPelicula = async () => {
    ...
    try {
        const respuesta = await fetch(`/api/content/v1/content/films/${encodeURIComponent(filmID)}`);
...
let editarPelicula = async () => {
    ...
    try {
        const peticion = await fetch(`/api/content/v1/content/films/${encodeURIComponent(pelicula.filmID)}`, {
            method: 'PUT',
            headers: {
