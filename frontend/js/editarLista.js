...
async function editarLista() {
    try {
        ...
        const filmsResponse = await fetch(`/api/user/v1/user/user/${userID}/list/${listID}/films`);
        const seriesResponse = await fetch(`/api/user/v1/user/user/${userID}/list/${listID}/series`);
        ...
        const listResponse = await fetch(`/api/user/v1/user/user/${userID}/list/${listID}`);
        if (!listResponse.ok) {
...
async function eliminarElemento(listID, tipo, itemID) {
    ...
    try {
        let url = null;
        if (tipo === 'film') {
            url = `/api/user/v1/user/user/${userID}/list/${listID}/film/${itemID}`;
        } else if (tipo === 'serie') {
            url = `/api/user/v1/user/user/${userID}/list/${listID}/serie/${itemID}`;
        }
...
async function guardarNombre() {
    ...
    const peticion = await fetch(`/api/user/v1/user/user/${userID}/list/${listID}`,
    {   method: 'PUT',
        headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
...
const obtenerInformacionLista = async () => {
    try {
        const response = await fetch(`/api/user/v1/user/user/${userID}/list/${listID}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
