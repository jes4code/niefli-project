...
let cargarDatosPerfil = async (profileID) => {
    ...
    try {
        const respuesta = await fetch(`/api/user/v1/user/user/${encodeURIComponent(userID)}/profiles/${encodeURIComponent(profileID)}`);
...
let editarPerfil = async () => {
    ...
    try {
        const peticion = await fetch(`/api/user/v1/user/user/${encodeURIComponent(userID)}/profiles/${encodeURIComponent(profileID)}`, {
