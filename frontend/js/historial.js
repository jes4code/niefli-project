...
window.onload = function() {
    ...
    document.getElementById("miBoton").addEventListener("click", borrarHistorial);

    verHistorial();
}

let verHistorial = async () => {
    try {
        const peticion = await fetch(`/api/visualization/views/visualizations/${userID}`, {
            method: 'GET',
            headers: {
...
const obtenerInformacionPeliculas = async (filmIDs) => {
    try {
        for (const filmID of filmIDs) {
            const peticion = await fetch(`/api/content/v1/content/films/${filmID}`, {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
...
const obtenerInformacionSeries = async (serieIDs) => {
    try {
        for (const serieID of serieIDs) {
            const peticion = await fetch(`/api/content/v1/content/series/${serieID}`, {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
