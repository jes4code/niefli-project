...
let cargarDatosEpisodio = async () => {
    ...
    try {
        const respuesta = await fetch(`/api/content/v1/content/series/${encodeURIComponent(serieID)}/episodes/${encodeURIComponent(episodeID)}`);
...
let editarEpisodio = async () => {
    ...
    try {
        const peticion = await fetch(`/api/content/v1/content/series/${encodeURIComponent(serieID)}/episodes/${encodeURIComponent(episodeID)}`, {
            method: 'PUT',
            headers: {
