...
let cargarDatosActor = async () => {
    ...
    try {
        const respuesta = await fetch(`/api/content/v1/content/actors/${encodeURIComponent(actorID)}`);
...
let editarActor = async () => {
    ...
    try {
        const peticion = await fetch(`/api/content/v1/content/actors/${encodeURIComponent(actor.actorID)}`, {
            method: 'PUT',
            headers: {
