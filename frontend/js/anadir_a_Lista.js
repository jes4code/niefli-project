...
const obtenerInformacionCuenta = async (userID) => {
    try {
        const response = await fetch(`/api/user/v1/user/user/${userID}`, {
            method: 'GET',
            headers: {
...
const obtenerNombresPeliculas = async (films) => {
    try {
        ...
        const nombresPeliculas = await Promise.all(
            films.map(async (id) => {
                const response = await fetch(`/api/content/v1/content/films/${id}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
...
const obtenerNombresSeries = async (series) => {
    try {
        ...
        const nombresSeries = await Promise.all(
            series.map(async (id) => {
                const response = await fetch(`/api/content/v1/content/series/${id}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
...
let listarListas = async () => {
    try {
        const peticion = await fetch(`/api/user/v1/user/user/${userID}/lists`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
...
let anadirLista = async (listID) => {
    try {
        ...
        let url = null;
        if (filmID) {
            url = `/api/user/v1/user/user/${userID}/list/${listID}/film/${filmID}`;
        } else if (serieID) {
            url = `/api/user/v1/user/user/${userID}/list/${listID}/serie/${serieID}`;
        } else {
