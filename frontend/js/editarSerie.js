...
let cargarDatosSerie = async () => {
    ...
    try {
        const respuesta = await fetch(`/api/content/v1/content/series/${encodeURIComponent(serieID)}`);
...
let editarSerie = async () => {
    ...
    try {
        const peticion = await fetch(`/api/content/v1/content/series/${encodeURIComponent(serie.serieID)}`, {
            method: 'PUT',
            headers: {
