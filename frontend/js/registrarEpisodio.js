...
let registrarEpisodio = async (serieID) => {
    ...
    const episodeID = Number(document.getElementById("episodeID").value); // o parseInt(...,10)
    const titulo = document.getElementById("titulo").value.trim();
    const numEpisodio = parseInt(document.getElementById("numEpisodio").value.trim(), 10);
    const numTemporada = parseInt(document.getElementById("numTemporada").value.trim(), 10);
    ...
    // Validar requeridos
    if (!titulo || Number.isNaN(numEpisodio) || Number.isNaN(numTemporada) || !photoURL) {
    ...
    try {
        const peticion = await fetch(`/api/content/v1/content/series/${encodeURIComponent(serieID)}/episodes`, {
            method: 'POST',
            headers: {
