document.addEventListener('DOMContentLoaded', () => {
    ...
    const obtenerPerfiles = async (userID) => {
        try {
            ...
            const response = await fetch(`/api/user/v1/user/user/${userID}/profiles`, {
                method: 'GET',
                headers: {
    ...
    const eliminarPerfil = async (profileID) => {
        try {
            const response = await fetch(`/api/user/v1/user/user/${encodeURIComponent(userID)}/profiles/${encodeURIComponent(profileID)}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
