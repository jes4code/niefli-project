...
let listarPeliculas = async () => {
    try {
        const response = await fetch(`/api/visualization/views/recommendations/film/${userID}`, {
            method: 'GET',
            headers: {
...
let listarSeries = async () => {
    try {
        const peticion = await fetch(`/api/visualization/views/recommendations/serie/${userID}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
